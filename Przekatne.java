...
public class Przekatne {
    public static void main(String[] args) {
        ...
        for(int i = 0; i<tabl.length; i++){
            przekatnaPrawa = przekatnaPrawa + tabl[i][i];       // wlasciwosc przekatnej - indeksy sa rowne
        }
        for(int i = 0; i<tabl.length; i++){
            przekatnaLewa = przekatnaLewa + tabl[i][tabl.length-i-1];     // wlasciwosc anti-przekatnej - jeden od poczatku drugi od konca
        }
